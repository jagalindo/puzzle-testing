...
public class FMStatistics {
	...
	private boolean isOr(Relation r){
		...
		int numCardinalities = 0;
		Iterator<Cardinality> it = r.getCardinalities();
		while (it.hasNext()) {
			it.next();
			numCardinalities ++;
		}
		
		if (numCardinalities == 1) {
			it = r.getCardinalities();
			if (it.hasNext()) {
				Cardinality card = (Cardinality) it.next();
				res = (card.getMin() == 0) || (card.getMin() == 1);

				int destinationNumber = 0;
				Iterator<Feature> itDestinations = r.getDestination();
				while (itDestinations.hasNext()) {
					itDestinations.next();
					destinationNumber++;
				}
				res = res && destinationNumber > 1;
			}
