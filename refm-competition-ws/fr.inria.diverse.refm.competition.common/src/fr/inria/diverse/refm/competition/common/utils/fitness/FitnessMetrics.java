...
public class FitnessMetrics {
	...
	public FitnessMetricsVO compute(VariabilityModel fm, Graph<Vertex, Arc> dependenciesGraph, String PCM) {
		...
		double numberOfFMProducts = computeNumberOfProducts(fm);
		double numberOfPCMProducts = computeNumberOfProducts(PCM);
		
		// Computing precision
		double intersection = computeIntersectionPCMFM(fm, PCM);
		double precision = (double)intersection / (double)numberOfFMProducts;
		metrics.setPrecision(precision);
		
		// Computing recall
		double recall = (double)intersection / (double)numberOfPCMProducts;
	...
	public double computeNumberOfProducts(VariabilityModel fm) {
	...
	public double computeNumberOfProducts(String PCM) {
		return PCM.split("\n").length - 1;
	}

	private double computeIntersectionPCMFM(VariabilityModel fm, String PCM) {
		this.computeProducts(fm);
